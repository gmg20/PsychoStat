...
m1 <- lm(Recall ~ Condition, data = dat1) # Fit Regression Mod
...
eta_squared(fitlm1, partial = FALSE) # Partial eta squared & semi-partial squared
...
SME_cond_old <- aov(Recall ~ Condition, data = Age.O) # Fit SME old

# Simple Main Effect of Condition for Young Folks

SME_cond_young <- aov(Recall ~ Condition, data = Age.Y) # Fit SME young
