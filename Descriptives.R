...
p1b<-ggplot(dat1,aes(StudyHrs, color=Gender, fill=Gender)) + geom_histogram(alpha=0.3)
p1b
p1b + ggtitle("Histogram of Study Hrs by Gender")


p1c<-ggplot(dat1, aes(StudyHrs, color=Gender, fill=Gender)) + geom_histogram(alpha = 0.3) + facet_grid(~Gender)
p1c
p1c + ggtitle("Histogram of Study Hours by Gender")

# Neuroticism by School Year
p1d<-ggplot(dat2, aes(neurot,color=year, fill=year)) + geom_histogram(alpha = 0.3)
p1d + ggtitle("Histogram of Neuroticism by School Year")

p1e<-ggplot(dat2, aes(stress, color=year, fill=year)) + geom_histogram(alpha=0.3)
...
p2<-ggplot(dat1,aes(StudyHrs)) + geom_boxplot(alpha=0.3) # Study Hrs Overall
p2
p2 + ggtitle("Study Hours Boxplot")

p2b<-ggplot(dat1, aes(Gender, StudyHrs, fill=Gender)) + geom_boxplot(alpha=0.3) # By Gender
p2b
p2b + ggtitle("Study Hours by Gender Boxplot")

p2c<-ggplot(dat1, aes(Gender, StudyHrs, fill=Gender)) + geom_boxplot(alpha=0.3) + facet_grid(~Gender)
p2c
p2c + ggtitle("Study Hours by Gender Boxplot")
#################################################################################
# Density Plots
p2d<-ggplot(dat1, aes(StudyHrs, color=Gender, fill=Gender)) + geom_density(alpha=0.3)
p2d + ggtitle("Study Hours by Gender")

p2e<-ggplot(dat2, aes(neurot, color=year, fill=year))+geom_density(alpha=0.3) # Neurot by Year
